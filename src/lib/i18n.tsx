import { createContext, useContext, useState, useEffect, ReactNode } from "react";

export type Lang = "es" | "en";

interface I18nContextType {
  lang: Lang;
  setLang: (lang: Lang) => void;
  t: (key: string) => string;
}

const I18nContext = createContext<I18nContextType | null>(null);

function detectLanguage(): Lang {
  const stored = localStorage.getItem("lang");
  if (stored === "es" || stored === "en") return stored;
  const nav = navigator.language?.toLowerCase() || "";
  return nav.startsWith("es") ? "es" : "en";
}

const translations: Record<Lang, Record<string, string>> = {
  es: {
    // Nav & Common
    "nav.login": "Iniciar sesión",
    "nav.logout": "Salir",
    "nav.profile": "Perfil",
    "common.next": "Siguiente →",
    "common.prev": "← Anterior",
    "common.save": "Guardar cambios",
    "common.saving": "Guardando...",
    "common.loading": "Cargando...",
    "common.none": "Ninguna",
    "common.active": "Activa",
    "common.inactive": "Inactiva",
    "common.cancel": "Cancelar",
    "common.share": "Compartir",
    "common.download": "Descargar",
    "common.viewPlan": "Ver plan completo →",

    // Landing
    "landing.hero": "Tu chef personal crea hasta",
    "landing.heroHighlight": "28 recetas",
    "landing.heroEnd": "listas para cocinar en minutos.",
    "landing.sub": "¿Qué cocino hoy? Ya no más esa pregunta. Paraguachi Meals Prep — fácil, rápido y delicioso.",
    "landing.pricingTitle": "Elige el plan perfecto para ti",
    "landing.pricing": "Suscripción mensual",
    "landing.priceMonth": "/mes",
    "landing.cancelAnytime": "Cancela cuando quieras · Sin compromiso",
    "landing.cta": "Comenzar ahora →",
    "landing.bestValue": "Mejor precio",
    "landing.planWeeklyLabel": "Semanal",
    "landing.planWeeklyPeriod": "/semana",
    "landing.planWeeklyPerDay": "~$1.43/día",
    "landing.planBiweeklyLabel": "Quincenal",
    "landing.planBiweeklyPeriod": "/2 semanas",
    "landing.planBiweeklyPerDay": "~$1.43/día",
    "landing.planMonthlyLabel": "Mensual",
    "landing.planMonthlyPeriod": "/mes",
    "landing.planMonthlyPerDay": "~$1.17/día — Ahorras más",
    "landing.featuresTitle": "Todo lo que necesitas para comer mejor",
    "landing.f1": "Plan de 28 días personalizado con tus ingredientes",
    "landing.f2": "Foto de recibo → menú completo en segundos",
    "landing.f3": "Recetas paso a paso para principiantes",
    "landing.f4": "Lista de compras semanal automática",
    "landing.f5": "Macros y nutrición calculados por receta",
    "landing.f6": "Guía de meal prep + consejos de almacenamiento",
    "landing.f7": "Soporte para dietas especiales y alergias",
    "landing.f8": "En español e inglés",

    // Login
    "login.title": "Iniciar sesión",
    "login.subtitle": "Ingresa a tu cuenta",
    "login.email": "Email",
    "login.password": "Contraseña",
    "login.button": "Iniciar sesión",
    "login.noAccount": "¿No tienes cuenta?",
    "login.register": "Regístrate",
    "login.error": "Error al iniciar sesión: ",
    "login.forgot": "¿Olvidaste tu contraseña?",
    "login.forgotSent": "Te enviamos un enlace de recuperación a tu email.",
    "login.forgotError": "Error al enviar enlace: ",

    // Register
    "register.title": "Crear cuenta",
    "register.subtitle": "Comienza tu plan de comidas",
    "register.name": "Nombre",
    "register.button": "Crear cuenta",
    "register.hasAccount": "¿Ya tienes cuenta?",
    "register.login": "Inicia sesión",
    "register.success": "¡Cuenta creada! Redirigiendo...",
    "register.error": "Error al registrarse: ",

    // Checkout
    "checkout.title": "Elige tu plan",
    "checkout.subtitle": "Cancela cuando quieras. Sin compromiso.",
    "checkout.plan": "Plan mensual — $35/mes",
    "checkout.payButton": "Suscribirme ahora",
    "checkout.secure": "Pago seguro procesado por Stripe. Cancela cuando quieras.",
    "checkout.f1": "Planes de 28 días ilimitados",
    "checkout.f2": "Recetas únicas personalizadas con IA",
    "checkout.f3": "Listas de compras y guía de meal prep",
    "checkout.f4": "Exportación HTML y compartir planes",
    "checkout.error": "Error al procesar el pago: ",
    "checkout.activeTitle": "¡Suscripción activa!",
    "checkout.activeDesc": "Tu plan está activo. Comienza a generar tus recetas.",
    "checkout.goOnboarding": "Ir al onboarding →",
    "checkout.needAccount": "¿No tienes cuenta?",
    "checkout.manage": "Gestionar suscripción",
    "checkout.popular": "Popular",
    "checkout.priceWeekly": "$10",
    "checkout.periodWeekly": "por semana",
    "checkout.priceBiweekly": "$20",
    "checkout.periodBiweekly": "cada 2 semanas",
    "checkout.priceMonthly": "$35",
    "checkout.periodMonthly": "por mes",

    // Onboarding
    "onboarding.title": "Personaliza tu plan",
    "onboarding.step": "Paso",
    "onboarding.of": "de",
    "onboarding.step1.title": "¿Cómo quieres ingresar tus ingredientes?",
    "onboarding.step1.receipt": "Foto del Recibo",
    "onboarding.step1.fridge": "Foto de Nevera",
    "onboarding.step1.manual": "Lista Manual",
    "onboarding.step1.help": "Ayúdame a Comprar",
    "onboarding.step1.upload": "Sube tu foto aquí",
    "onboarding.step1.camera": "Cámara",
    "onboarding.step1.gallery": "Galería",
    "onboarding.step1.analyzing": "Analizando ingredientes con IA...",
    "onboarding.step1.detected": "Ingredientes detectados",
    "onboarding.step1.added": "Ingredientes agregados a la lista",
    "onboarding.step1.addAll": "Agregar todos a mi lista",
    "onboarding.step1.placeholder": "Escribe tus ingredientes, uno por línea...",
    "onboarding.step2.title": "Selecciona tus alimentos",
    "onboarding.step2.animalProteins": "🥩 Proteínas Animales",
    "onboarding.step2.plantProteins": "🌱 Proteínas Vegetales",
    "onboarding.step2.dairy": "🧀 Lácteos",
    "onboarding.step2.grains": "🌾 Granos",
    "onboarding.step2.veggies": "🥬 Vegetales",
    "onboarding.step2.fruits": "🍎 Frutas",
    "onboarding.step2.spices": "🧂 Condimentos",
    "onboarding.step2.addOther": "Agregar otro",
    "onboarding.step2.add": "Agregar",
    "onboarding.step3.title": "¿Para cuántas personas?",
    "onboarding.step3.meals": "Comidas del día",
    "onboarding.step4.title": "¿Cuál es tu objetivo?",
    "onboarding.body.title": "📊 Tus medidas corporales",
    "onboarding.body.subtitle": "Como nutricionista, necesito estos datos para calcular tus necesidades calóricas",
    "onboarding.body.weight": "Peso actual (kg)",
    "onboarding.body.height": "Estatura (cm)",
    "onboarding.body.age": "Edad",
    "onboarding.body.sex": "Sexo biológico",
    "onboarding.body.male": "Masculino",
    "onboarding.body.female": "Femenino",
    "onboarding.body.activity": "Nivel de actividad física",
    "onboarding.body.sedentary": "Sedentario (oficina)",
    "onboarding.body.light": "Ligero (caminar 1-3 días)",
    "onboarding.body.moderate": "Moderado (ejercicio 3-5 días)",
    "onboarding.body.active": "Activo (deporte 6-7 días)",
    "onboarding.body.veryActive": "Muy activo (atleta / trabajo físico)",
    "onboarding.body.goalWeight": "Meta de peso (kg)",
    "onboarding.body.dietType": "¿Cómo prefieres medir tu comida?",
    "onboarding.body.byKcal": "Por calorías (kcal)",
    "onboarding.body.byPortion": "Por porciones / peso",
    "onboarding.body.bmi": "Tu IMC",
    "onboarding.body.bmiUnderweight": "Bajo peso",
    "onboarding.body.bmiNormal": "Normal",
    "onboarding.body.bmiOverweight": "Sobrepeso",
    "onboarding.body.bmiObese": "Obesidad",
    "onboarding.body.tdee": "Gasto calórico estimado",
    "onboarding.body.kcalDay": "kcal/día",
    "onboarding.step8.weight": "Peso",
    "onboarding.step8.heightLabel": "Estatura",
    "onboarding.step8.bmi": "IMC",
    "onboarding.step8.goalWeight": "Meta de peso",
    "onboarding.step8.dietType": "Medición",
    "onboarding.step5.title": "Restricciones y preferencias",
    "onboarding.step5.allergies": "Alergias",
    "onboarding.step5.diets": "Dietas",
    "onboarding.step5.health": "Salud",
    "onboarding.step5.prefs": "Preferencias",
    "onboarding.step6.title": "Tu cocina",
    "onboarding.step6.time": "Tiempo de cocina",
    "onboarding.step6.level": "Nivel culinario",
    "onboarding.step6.equipment": "Equipamiento",
    "onboarding.step7.title": "¿Cuántas comidas quieres preparar?",
    "onboarding.step7.auto": "Auto (máximo)",
    "onboarding.step7.days": "días",
    "onboarding.step7.based": "Basado en tus ingredientes, podemos generar aproximadamente:",
    "onboarding.step7.upTo": "🍽️ Hasta",
    "onboarding.step7.uniqueRecipes": "recetas únicas",
    "onboarding.step8.title": "Resumen",
    "onboarding.step8.people": "Personas",
    "onboarding.step8.meals": "Comidas",
    "onboarding.step8.goal": "Objetivo",
    "onboarding.step8.restrictions": "Restricciones",
    "onboarding.step8.time": "Tiempo",
    "onboarding.step8.level": "Nivel",
    "onboarding.step8.equipment": "Equipamiento",
    "onboarding.generate": "🚀 Generar mi plan de",
    "onboarding.generating": "Generando tu plan personalizado...",
    "onboarding.success": "¡Plan generado exitosamente!",
    "onboarding.error": "Error al generar el plan: ",
    "onboarding.loginRequired": "Debes iniciar sesión",

    // Dashboard
    "dashboard.hello": "Hola,",
    "dashboard.plans": "planes generados",
    "dashboard.recipes": "recetas únicas",
    "dashboard.memberSince": "Miembro desde",
    "dashboard.newPlan": "Nuevo plan",
    "dashboard.lastPlan": "Mi último plan",
    "dashboard.myPlans": "Mis planes",
    "dashboard.noPlans": "Sin planes aún",
    "dashboard.noPlansDesc": "Genera tu primer plan de comidas personalizado",
    "dashboard.start": "Comenzar →",
    "dashboard.planOf": "Plan de",
    "dashboard.daysUnit": "días",
    "dashboard.connectTelegram": "Conecta Telegram",
    "dashboard.telegramDesc": "Recibe tus planes y recordatorios por Telegram",
    "dashboard.telegramConnect": "Conectar",
    "dashboard.telegramConnected": "✅ Telegram conectado",
    "dashboard.suggestions": "Para tu próximo plan, considera agregar:",
    "dashboard.linkCopied": "Enlace copiado al portapapeles",
    "dashboard.htmlDownloaded": "HTML descargado",
    "dashboard.htmlError": "Error al exportar HTML",

    // Plan Viewer
    "plan.title": "Plan de comidas",
    "plan.notFound": "Plan no encontrado",
    "plan.backDashboard": "← Volver al dashboard",
    "plan.week": "Semana",
    "plan.day": "Día",
    "plan.viewSteps": "Ver pasos",
    "plan.shopping": "Lista de compras",
    "plan.mealPrep": "Guía de Meal Prep",
    "plan.ingredients": "Ingredientes detectados",
    "plan.generated": "Recetas generadas",
    "plan.warnings": "⚠️ Advertencias",
    "plan.suggestions": "💡 Sugerencias para más variedad",
    "plan.additionalRecipes": "recetas",
    "plan.pdfDownloaded": "PDF descargado",
    "plan.pdfError": "Error al generar PDF",

    // Public Plan
    "public.shared": "Plan compartido",
    "public.expired": "Plan no encontrado o expirado",
    "public.ctaTitle": "¿Quieres tu plan personalizado?",
    "public.ctaDesc": "Obtén 28 recetas adaptadas a tus ingredientes y preferencias",
    "public.ctaButton": "Comenzar mi plan ahora →",
    "public.wantMyPlan": "Crear mi plan personalizado →",
    "public.wantPlan": "Quiero mi plan →",

    // Profile
    "profile.title": "Mi Perfil",
    "profile.photo": "Foto y Nombre",
    "profile.fullName": "Nombre completo",
    "profile.phone": "Teléfono",
    "profile.personal": "Datos Personales",
    "profile.birthdate": "Fecha de nacimiento",
    "profile.city": "Ciudad",
    "profile.country": "País",
    "profile.weight": "Peso (kg)",
    "profile.height": "Altura (cm)",
    "profile.healthTitle": "Salud y Restricciones",
    "profile.dietRestrictions": "Restricciones alimenticias",
    "profile.healthProblems": "Problemas de salud",
    "profile.subscription": "Mi Suscripción",
    "profile.nextCharge": "Próximo cobro:",
    "profile.security": "Seguridad",
    "profile.changePassword": "Cambiar contraseña",
    "profile.passwordEmail": "Revisa tu email para cambiar la contraseña",
    "profile.telegram": "Telegram",

    // Reset Password
    "reset.title": "Nueva contraseña",
    "reset.subtitle": "Ingresa tu nueva contraseña",
    "reset.newPassword": "Nueva contraseña",
    "reset.confirmPassword": "Confirmar contraseña",
    "reset.button": "Cambiar contraseña",
    "reset.mismatch": "Las contraseñas no coinciden",
    "reset.tooShort": "La contraseña debe tener al menos 6 caracteres",
    "reset.error": "Error al cambiar contraseña: ",
    "reset.success": "¡Contraseña actualizada!",
    "reset.invalid": "Enlace inválido o expirado. Solicita un nuevo enlace de recuperación.",
    "reset.doneTitle": "¡Listo!",
    "reset.doneDesc": "Tu contraseña fue actualizada. Redirigiendo...",
    "profile.connected": "Conectado",
    "profile.notConnected": "No conectado. Próximamente podrás vincular tu cuenta de Telegram.",
    "profile.saved": "Perfil actualizado",
    "profile.saveError": "Error al guardar",
    "profile.portalError": "Error al abrir el portal de suscripción",
    "profile.photoUpdated": "Foto actualizada",
    "profile.photoError": "Error al subir imagen",

    // Admin
    "admin.panel": "Panel de Administración",
    "admin.menu": "Menú",
    "admin.dashboard": "Dashboard",
    "admin.users": "Usuarios",
    "admin.subscriptions": "Suscripciones",
    "admin.recipes": "Recetas",
    "admin.aiUsage": "Uso de IA",
    "admin.connections": "Conexiones",
    "admin.branding": "Marca",
    "admin.currentLogo": "Logo actual",
    "admin.noLogo": "Sin logo personalizado",
    "admin.uploadLogo": "Subir logo",
    "admin.logoUpdated": "Logo actualizado correctamente",
    "admin.logoRemoved": "Logo eliminado",
    "admin.exit": "Salir",

    // Footer
    "footer.credits": "Paraguachi Meals Prep · Ing. Chef Alexander Matute & Ing. Nelly Rendón, Especialista en Manipulación y Conservación de Alimentos · Los Angeles, CA",
    "footer.disclaimer": "Los planes nutricionales generados por Paraguachi Meals Prep son de carácter orientativo y educativo. No constituyen consejo médico, diagnóstico ni tratamiento. No reemplazan la consulta con un médico, nutricionista o dietista certificado. Si padeces diabetes, hipertensión, enfermedad renal u otras condiciones médicas, consulta siempre a tu médico antes de realizar cambios en tu alimentación.",

    // Clinical onboarding
    "onboarding.clinical.title": "🏥 Datos clínicos",
    "onboarding.clinical.subtitle": "Información médica para personalizar tu plan (confidencial)",
    "onboarding.clinical.diabetesType": "Tipo de diabetes",
    "onboarding.clinical.diabetesNone": "No tengo",
    "onboarding.clinical.diabetesPre": "Prediabetes",
    "onboarding.clinical.diabetes1": "Tipo 1",
    "onboarding.clinical.diabetes2": "Tipo 2",
    "onboarding.clinical.insulin": "¿Usas insulina?",
    "onboarding.clinical.yes": "Sí",
    "onboarding.clinical.no": "No",
    "onboarding.clinical.hba1c": "HbA1c (%)",
    "onboarding.clinical.renalDisease": "¿Enfermedad renal?",
    "onboarding.clinical.systolic": "Presión sistólica (mmHg)",
    "onboarding.clinical.diastolic": "Presión diastólica (mmHg)",
    "onboarding.clinical.sodiumGoal": "Meta de sodio",
    "onboarding.clinical.sodiumStandard": "Estándar (2,300 mg)",
    "onboarding.clinical.sodiumStrict": "Estricto (1,500 mg)",
    "onboarding.clinical.medications": "Medicamentos actuales",
    "onboarding.clinical.medicationsPlaceholder": "Ej: Metformina, Losartán...",
    "onboarding.clinical.addMed": "Agregar",
    "onboarding.clinical.insulinResistance": "¿Resistencia a la insulina?",
    "onboarding.clinical.irYes": "Sí",
    "onboarding.clinical.irNo": "No",
    "onboarding.clinical.irUnknown": "No sé",
    "onboarding.clinical.sleepHours": "Horas de sueño",
    "onboarding.clinical.stressLevel": "Nivel de estrés",
    "onboarding.clinical.stressLow": "Bajo",
    "onboarding.clinical.stressMedium": "Medio",
    "onboarding.clinical.stressHigh": "Alto",
    "onboarding.clinical.stressVeryHigh": "Muy alto",
    "onboarding.clinical.optional": "Medidas opcionales",
    "onboarding.clinical.bodyFat": "% Grasa corporal",
    "onboarding.clinical.waist": "Cintura (cm)",
    "onboarding.clinical.hip": "Cadera (cm)",
    "onboarding.clinical.arm": "Brazo (cm)",
    "onboarding.clinical.bloodPressure": "Presión arterial",

    // Onboarding data labels
    "data.goal.sano": "Comer más sano",
    "data.goal.perder": "Perder grasa corporal",
    "data.goal.musculo": "Ganar músculo",
    "data.goal.mantener": "Mantener peso",
    "data.goal.familia": "Familia balanceada",
    "data.goal.rendimiento": "Alto rendimiento",
    "data.time.15": "15 min",
    "data.time.20-35": "20-35 min",
    "data.time.60": "Hasta 1 hora",
    "data.time.mealprep": "Meal Prep domingo",
    "data.skill.beginner": "Principiante",
    "data.skill.basic": "Básico",
    "data.skill.intermediate": "Intermedio",
    "data.skill.advanced": "Avanzado",
    "data.meal.breakfast": "Desayuno",
    "data.meal.midmorning": "Media Mañana",
    "data.meal.lunch": "Almuerzo",
    "data.meal.snack": "Merienda",
    "data.meal.dinner": "Cena",
    "data.meal.nightsnack": "Snack Nocturno",
  },
  en: {
    // Nav & Common
    "nav.login": "Sign in",
    "nav.logout": "Log out",
    "nav.profile": "Profile",
    "common.next": "Next →",
    "common.prev": "← Back",
    "common.save": "Save changes",
    "common.saving": "Saving...",
    "common.loading": "Loading...",
    "common.none": "None",
    "common.active": "Active",
    "common.inactive": "Inactive",
    "common.cancel": "Cancel",
    "common.share": "Share",
    "common.download": "Download",
    "common.viewPlan": "View full plan →",

    // Landing
    "landing.hero": "Your personal chef creates up to",
    "landing.heroHighlight": "28 recipes",
    "landing.heroEnd": "ready to cook in minutes.",
    "landing.sub": "What should I cook today? No more asking. Paraguachi Meals Prep — easy, fast, and delicious.",
    "landing.pricingTitle": "Choose the perfect plan for you",
    "landing.pricing": "Monthly subscription",
    "landing.priceMonth": "/month",
    "landing.cancelAnytime": "Cancel anytime · No commitment",
    "landing.cta": "Get started →",
    "landing.bestValue": "Best value",
    "landing.planWeeklyLabel": "Weekly",
    "landing.planWeeklyPeriod": "/week",
    "landing.planWeeklyPerDay": "~$1.43/day",
    "landing.planBiweeklyLabel": "Biweekly",
    "landing.planBiweeklyPeriod": "/2 weeks",
    "landing.planBiweeklyPerDay": "~$1.43/day",
    "landing.planMonthlyLabel": "Monthly",
    "landing.planMonthlyPeriod": "/month",
    "landing.planMonthlyPerDay": "~$1.17/day — You save more",
    "landing.featuresTitle": "Everything you need to eat better",
    "landing.f1": "28-day plan customized with your ingredients",
    "landing.f2": "Receipt photo → full menu in seconds",
    "landing.f3": "Step-by-step recipes for beginners",
    "landing.f4": "Automatic weekly shopping list",
    "landing.f5": "Macros and nutrition calculated per recipe",
    "landing.f6": "Meal prep guide + storage tips",
    "landing.f7": "Support for special diets and allergies",
    "landing.f8": "In English and Spanish",

    // Login
    "login.title": "Sign in",
    "login.subtitle": "Access your account",
    "login.email": "Email",
    "login.password": "Password",
    "login.button": "Sign in",
    "login.noAccount": "Don't have an account?",
    "login.register": "Sign up",
    "login.error": "Login error: ",
    "login.forgot": "Forgot your password?",
    "login.forgotSent": "We sent a recovery link to your email.",
    "login.forgotError": "Error sending link: ",

    // Register
    "register.title": "Create account",
    "register.subtitle": "Start your meal plan",
    "register.name": "Name",
    "register.button": "Create account",
    "register.hasAccount": "Already have an account?",
    "register.login": "Sign in",
    "register.success": "Account created! Redirecting...",
    "register.error": "Registration error: ",

    // Checkout
    "checkout.title": "Choose your plan",
    "checkout.subtitle": "Cancel anytime. No commitment.",
    "checkout.plan": "Monthly plan — $35/month",
    "checkout.payButton": "Subscribe now",
    "checkout.secure": "Secure payment processed by Stripe. Cancel anytime.",
    "checkout.f1": "Unlimited 28-day plans",
    "checkout.f2": "AI-personalized unique recipes",
    "checkout.f3": "Shopping lists and meal prep guide",
    "checkout.f4": "HTML export and plan sharing",
    "checkout.error": "Payment error: ",
    "checkout.activeTitle": "Subscription active!",
    "checkout.activeDesc": "Your plan is active. Start generating your recipes.",
    "checkout.goOnboarding": "Go to onboarding →",
    "checkout.needAccount": "Don't have an account?",
    "checkout.manage": "Manage subscription",
    "checkout.popular": "Popular",
    "checkout.priceWeekly": "$10",
    "checkout.periodWeekly": "per week",
    "checkout.priceBiweekly": "$20",
    "checkout.periodBiweekly": "every 2 weeks",
    "checkout.priceMonthly": "$35",
    "checkout.periodMonthly": "per month",

    // Onboarding
    "onboarding.title": "Customize your plan",
    "onboarding.step": "Step",
    "onboarding.of": "of",
    "onboarding.step1.title": "How do you want to enter your ingredients?",
    "onboarding.step1.receipt": "Receipt Photo",
    "onboarding.step1.fridge": "Fridge Photo",
    "onboarding.step1.manual": "Manual List",
    "onboarding.step1.help": "Help Me Shop",
    "onboarding.step1.upload": "Upload your photo here",
    "onboarding.step1.camera": "Camera",
    "onboarding.step1.gallery": "Gallery",
    "onboarding.step1.analyzing": "Analyzing ingredients with AI...",
    "onboarding.step1.detected": "Detected ingredients",
    "onboarding.step1.added": "Ingredients added to list",
    "onboarding.step1.addAll": "Add all to my list",
    "onboarding.step1.placeholder": "Type your ingredients, one per line...",
    "onboarding.step2.title": "Select your foods",
    "onboarding.step2.animalProteins": "🥩 Animal Proteins",
    "onboarding.step2.plantProteins": "🌱 Plant Proteins",
    "onboarding.step2.dairy": "🧀 Dairy",
    "onboarding.step2.grains": "🌾 Grains",
    "onboarding.step2.veggies": "🥬 Vegetables",
    "onboarding.step2.fruits": "🍎 Fruits",
    "onboarding.step2.spices": "🧂 Spices & Condiments",
    "onboarding.step2.addOther": "Add other",
    "onboarding.step2.add": "Add",
    "onboarding.step3.title": "How many people?",
    "onboarding.step3.meals": "Meals of the day",
    "onboarding.step4.title": "What's your goal?",
    "onboarding.body.title": "📊 Your body metrics",
    "onboarding.body.subtitle": "As a nutritionist, I need this data to calculate your caloric needs",
    "onboarding.body.weight": "Current weight (kg)",
    "onboarding.body.height": "Height (cm)",
    "onboarding.body.age": "Age",
    "onboarding.body.sex": "Biological sex",
    "onboarding.body.male": "Male",
    "onboarding.body.female": "Female",
    "onboarding.body.activity": "Physical activity level",
    "onboarding.body.sedentary": "Sedentary (office work)",
    "onboarding.body.light": "Light (walk 1-3 days)",
    "onboarding.body.moderate": "Moderate (exercise 3-5 days)",
    "onboarding.body.active": "Active (sports 6-7 days)",
    "onboarding.body.veryActive": "Very active (athlete / physical job)",
    "onboarding.body.goalWeight": "Goal weight (kg)",
    "onboarding.body.dietType": "How do you prefer to measure your food?",
    "onboarding.body.byKcal": "By calories (kcal)",
    "onboarding.body.byPortion": "By portions / weight",
    "onboarding.body.bmi": "Your BMI",
    "onboarding.body.bmiUnderweight": "Underweight",
    "onboarding.body.bmiNormal": "Normal",
    "onboarding.body.bmiOverweight": "Overweight",
    "onboarding.body.bmiObese": "Obese",
    "onboarding.body.tdee": "Estimated daily calorie burn",
    "onboarding.body.kcalDay": "kcal/day",
    "onboarding.step8.weight": "Weight",
    "onboarding.step8.heightLabel": "Height",
    "onboarding.step8.bmi": "BMI",
    "onboarding.step8.goalWeight": "Goal weight",
    "onboarding.step8.dietType": "Measurement",
    "onboarding.step5.title": "Restrictions & preferences",
    "onboarding.step5.allergies": "Allergies",
    "onboarding.step5.diets": "Diets",
    "onboarding.step5.health": "Health",
    "onboarding.step5.prefs": "Preferences",
    "onboarding.step6.title": "Your kitchen",
    "onboarding.step6.time": "Cooking time",
    "onboarding.step6.level": "Skill level",
    "onboarding.step6.equipment": "Equipment",
    "onboarding.step7.title": "How many meals do you want to prepare?",
    "onboarding.step7.auto": "Auto (maximum)",
    "onboarding.step7.days": "days",
    "onboarding.step7.based": "Based on your ingredients, we can generate approximately:",
    "onboarding.step7.upTo": "🍽️ Up to",
    "onboarding.step7.uniqueRecipes": "unique recipes",
    "onboarding.step8.title": "Summary",
    "onboarding.step8.people": "People",
    "onboarding.step8.meals": "Meals",
    "onboarding.step8.goal": "Goal",
    "onboarding.step8.restrictions": "Restrictions",
    "onboarding.step8.time": "Time",
    "onboarding.step8.level": "Level",
    "onboarding.step8.equipment": "Equipment",
    "onboarding.generate": "🚀 Generate my",
    "onboarding.generating": "Generating your custom plan...",
    "onboarding.success": "Plan generated successfully!",
    "onboarding.error": "Error generating plan: ",
    "onboarding.loginRequired": "You must be logged in",

    // Dashboard
    "dashboard.hello": "Hi,",
    "dashboard.plans": "plans generated",
    "dashboard.recipes": "unique recipes",
    "dashboard.memberSince": "Member since",
    "dashboard.newPlan": "New plan",
    "dashboard.lastPlan": "My last plan",
    "dashboard.myPlans": "My plans",
    "dashboard.noPlans": "No plans yet",
    "dashboard.noPlansDesc": "Generate your first personalized meal plan",
    "dashboard.start": "Get started →",
    "dashboard.planOf": "Plan of",
    "dashboard.daysUnit": "days",
    "dashboard.connectTelegram": "Connect Telegram",
    "dashboard.telegramDesc": "Receive your plans and reminders via Telegram",
    "dashboard.telegramConnect": "Connect",
    "dashboard.telegramConnected": "✅ Telegram connected",
    "dashboard.suggestions": "For your next plan, consider adding:",
    "dashboard.linkCopied": "Link copied to clipboard",
    "dashboard.htmlDownloaded": "HTML downloaded",
    "dashboard.htmlError": "Error exporting HTML",

    // Plan Viewer
    "plan.title": "Meal plan",
    "plan.notFound": "Plan not found",
    "plan.backDashboard": "← Back to dashboard",
    "plan.week": "Week",
    "plan.day": "Day",
    "plan.viewSteps": "View steps",
    "plan.shopping": "Shopping list",
    "plan.mealPrep": "Meal Prep Guide",
    "plan.ingredients": "Ingredients detected",
    "plan.generated": "Recipes generated",
    "plan.warnings": "⚠️ Warnings",
    "plan.suggestions": "💡 Suggestions for more variety",
    "plan.additionalRecipes": "recipes",
    "plan.pdfDownloaded": "PDF downloaded",
    "plan.pdfError": "Error generating PDF",

    // Public Plan
    "public.shared": "Shared plan",
    "public.expired": "Plan not found or expired",
    "public.ctaTitle": "Want your personalized plan?",
    "public.ctaDesc": "Get 28 recipes tailored to your ingredients and preferences",
    "public.ctaButton": "Start my plan now →",
    "public.wantMyPlan": "Create my personalized plan →",
    "public.wantPlan": "I want my plan →",

    // Profile
    "profile.title": "My Profile",
    "profile.photo": "Photo & Name",
    "profile.fullName": "Full name",
    "profile.phone": "Phone",
    "profile.personal": "Personal Data",
    "profile.birthdate": "Date of birth",
    "profile.city": "City",
    "profile.country": "Country",
    "profile.weight": "Weight (kg)",
    "profile.height": "Height (cm)",
    "profile.healthTitle": "Health & Restrictions",
    "profile.dietRestrictions": "Dietary restrictions",
    "profile.healthProblems": "Health conditions",
    "profile.subscription": "My Subscription",
    "profile.nextCharge": "Next charge:",
    "profile.security": "Security",
    "profile.changePassword": "Change password",
    "profile.passwordEmail": "Check your email to change your password",
    "profile.telegram": "Telegram",

    // Reset Password
    "reset.title": "New password",
    "reset.subtitle": "Enter your new password",
    "reset.newPassword": "New password",
    "reset.confirmPassword": "Confirm password",
    "reset.button": "Change password",
    "reset.mismatch": "Passwords don't match",
    "reset.tooShort": "Password must be at least 6 characters",
    "reset.error": "Error changing password: ",
    "reset.success": "Password updated!",
    "reset.invalid": "Invalid or expired link. Request a new recovery link.",
    "reset.doneTitle": "Done!",
    "reset.doneDesc": "Your password was updated. Redirecting...",
    "profile.connected": "Connected",
    "profile.notConnected": "Not connected. Telegram linking coming soon.",
    "profile.saved": "Profile updated",
    "profile.saveError": "Error saving",
    "profile.portalError": "Error opening subscription portal",
    "profile.photoUpdated": "Photo updated",
    "profile.photoError": "Error uploading image",

    // Admin
    "admin.panel": "Admin Panel",
    "admin.menu": "Menu",
    "admin.dashboard": "Dashboard",
    "admin.users": "Users",
    "admin.subscriptions": "Subscriptions",
    "admin.recipes": "Recipes",
    "admin.aiUsage": "AI Usage",
    "admin.connections": "Connections",
    "admin.branding": "Branding",
    "admin.currentLogo": "Current logo",
    "admin.noLogo": "No custom logo set",
    "admin.uploadLogo": "Upload logo",
    "admin.logoUpdated": "Logo updated successfully",
    "admin.logoRemoved": "Logo removed",
    "admin.exit": "Exit",

    // Footer
    "footer.credits": "Paraguachi Meals Prep · Chef Eng. Alexander Matute & Eng. Nelly Rendón, Food Handling & Preservation Specialist · Los Angeles, CA",
    "footer.disclaimer": "The nutritional plans generated by Paraguachi Meals Prep are for guidance and educational purposes only. They do not constitute medical advice, diagnosis, or treatment. They do not replace consultation with a physician, nutritionist, or certified dietitian. If you have diabetes, hypertension, kidney disease, or other medical conditions, always consult your doctor before making changes to your diet.",

    // Clinical onboarding
    "onboarding.clinical.title": "🏥 Clinical data",
    "onboarding.clinical.subtitle": "Medical information to customize your plan (confidential)",
    "onboarding.clinical.diabetesType": "Diabetes type",
    "onboarding.clinical.diabetesNone": "I don't have it",
    "onboarding.clinical.diabetesPre": "Prediabetes",
    "onboarding.clinical.diabetes1": "Type 1",
    "onboarding.clinical.diabetes2": "Type 2",
    "onboarding.clinical.insulin": "Do you use insulin?",
    "onboarding.clinical.yes": "Yes",
    "onboarding.clinical.no": "No",
    "onboarding.clinical.hba1c": "HbA1c (%)",
    "onboarding.clinical.renalDisease": "Kidney disease?",
    "onboarding.clinical.systolic": "Systolic pressure (mmHg)",
    "onboarding.clinical.diastolic": "Diastolic pressure (mmHg)",
    "onboarding.clinical.sodiumGoal": "Sodium goal",
    "onboarding.clinical.sodiumStandard": "Standard (2,300 mg)",
    "onboarding.clinical.sodiumStrict": "Strict (1,500 mg)",
    "onboarding.clinical.medications": "Current medications",
    "onboarding.clinical.medicationsPlaceholder": "E.g.: Metformin, Losartan...",
    "onboarding.clinical.addMed": "Add",
    "onboarding.clinical.insulinResistance": "Insulin resistance?",
    "onboarding.clinical.irYes": "Yes",
    "onboarding.clinical.irNo": "No",
    "onboarding.clinical.irUnknown": "Not sure",
    "onboarding.clinical.sleepHours": "Hours of sleep",
    "onboarding.clinical.stressLevel": "Stress level",
    "onboarding.clinical.stressLow": "Low",
    "onboarding.clinical.stressMedium": "Medium",
    "onboarding.clinical.stressHigh": "High",
    "onboarding.clinical.stressVeryHigh": "Very high",
    "onboarding.clinical.optional": "Optional measurements",
    "onboarding.clinical.bodyFat": "Body fat %",
    "onboarding.clinical.waist": "Waist (cm)",
    "onboarding.clinical.hip": "Hip (cm)",
    "onboarding.clinical.arm": "Arm (cm)",
    "onboarding.clinical.bloodPressure": "Blood pressure",

    // Onboarding data labels
    "data.goal.sano": "Eat healthier",
    "data.goal.perder": "Lose body fat",
    "data.goal.musculo": "Build muscle",
    "data.goal.mantener": "Maintain weight",
    "data.goal.familia": "Balanced family",
    "data.goal.rendimiento": "High performance",
    "data.time.15": "15 min",
    "data.time.20-35": "20-35 min",
    "data.time.60": "Up to 1 hour",
    "data.time.mealprep": "Sunday Meal Prep",
    "data.skill.beginner": "Beginner",
    "data.skill.basic": "Basic",
    "data.skill.intermediate": "Intermediate",
    "data.skill.advanced": "Advanced",
    "data.meal.breakfast": "Breakfast",
    "data.meal.midmorning": "Mid-Morning",
    "data.meal.lunch": "Lunch",
    "data.meal.snack": "Snack",
    "data.meal.dinner": "Dinner",
    "data.meal.nightsnack": "Night Snack",

    // Paraguachi Fit
    "fit.title": "Paraguachi Fit",
    "fit.subtitle": "Monthly training planner aligned with your meals",
    "fit.monthlyOnly": "Available exclusively for monthly subscribers",
    "fit.upgrade": "Upgrade to monthly to unlock",
    "fit.activate": "Generate my routine",
    "fit.regenerate": "Generate new routine",
    "fit.generating": "Designing your routine...",
    "fit.generated": "Routine ready!",
    "fit.error": "Could not generate routine",
    "fit.tabRoutine": "Routine",
    "fit.tabSessions": "Sessions",
    "fit.tabProgress": "Progress",
    "fit.daysWeek": "days/week",
    "fit.duration": "Duration",
    "fit.weeks": "weeks",
    "fit.week": "Week",
    "fit.session": "Session",
    "fit.warmup": "Warm-up",
    "fit.cooldown": "Cool-down",
    "fit.exercises": "Exercises",
    "fit.macroAdjust": "Macro tip",
    "fit.tips": "General tips",
    "fit.logSession": "Log a workout",
    "fit.uploadScreenshot": "Upload screenshot (Apple Watch, Fitbit, Garmin...)",
    "fit.analyzing": "Analyzing with AI...",
    "fit.aiDetected": "AI detected",
    "fit.kcal": "kcal",
    "fit.km": "km",
    "fit.minutes": "min",
    "fit.type": "Type",
    "fit.device": "Device",
    "fit.notes": "Notes",
    "fit.save": "Save session",
    "fit.savedSession": "Session logged",
    "fit.noSessions": "No sessions logged yet",
    "fit.uploadProgress": "Upload progress photo",
    "fit.photoType": "Photo type",
    "fit.photoStart": "Initial",
    "fit.photoProgress": "Progress",
    "fit.photoCurrent": "Current",
    "fit.weight": "Weight (kg)",
    "fit.waist": "Waist (cm)",
    "fit.savedPhoto": "Photo uploaded",
    "fit.noPhotos": "No progress photos yet",
    "fit.deviceConnect": "Device integrations",
    "fit.deviceComingSoon": "Direct sync coming soon",
    "fit.uploadAnyDevice": "Upload screenshots from any device — AI extracts your data automatically",
    "fit.dashboardCard": "Paraguachi Fit",
    "fit.dashboardOpen": "Open training",
    "fit.dashboardDesc": "Your monthly routine aligned with your meals",
    "fit.location": "Location",
    "fit.intensity": "Intensity",
    "fit.indoor": "Indoor",
    "fit.outdoor": "Outdoor",
    "fit.intLow": "Low",
    "fit.intModerate": "Moderate",
    "fit.intHigh": "High",
    "fit.selectType": "Select activity",
    "fit.manualHint": "Pick the activity type so we can adjust your macros accurately",
    "fit.sessionDuration": "Session duration",
    "fit.express": "Express",
    "fit.days": "days",
    "fit.watchDemo": "Watch demo",
    "fit.rest": "Rest",
    "fit.howTo": "How to perform it",
    "fit.demoHint": "Open in YouTube or MuscleWiki to watch real demonstrations",
    "fit.openYouTube": "Watch on YouTube",
    "fit.openMuscleWiki": "Browse on MuscleWiki",
    "fit.gender": "Gender",
    "fit.male": "Male",
    "fit.female": "Female",
    "fit.level": "Level",
    "fit.levelBeginner": "Beginner",
    "fit.levelIntermediate": "Intermediate",
    "fit.levelAdvanced": "Advanced",
    "fit.yourGoal": "Your current goal",
    "fit.setDesiredWeight": "Set desired weight",
    "fit.editGoal": "Edit",
    "fit.desiredWeight": "Desired weight",
    "fit.startWeight": "Initial",
    "fit.currentWeight": "Current",
    "fit.goalProgress": "Progress to goal",
    "fit.weightChange": "{kg} kg vs initial",
    "fit.noWeightChange": "No weight change yet",
    "fit.remaining": "{kg} kg to go",
    "fit.height": "Height",
    "fit.dailyKcal": "Daily target",
    "fit.bmiUnder": "underweight",
    "fit.bmiNormal": "healthy",
    "fit.bmiOver": "overweight",
    "fit.bmiObese": "obesity",
    "fit.last30Days": "Last 30 days",
    "fit.statSessions": "Sessions",
    "fit.statTime": "Total time",
    "fit.statKcal": "Burned",
    "fit.statKm": "Distance",
    "fit.perWeek": "/week",
    "fit.technicalAnalysis": "Technical analysis",
    "fit.analysisLosing": "You are losing weight steadily — your caloric deficit is working as planned.",
    "fit.analysisGainingFatGoal": "Weight is going up despite your fat-loss goal. Review portions and adherence to the plan.",
    "fit.analysisStable": "Weight is stable. If you want faster fat loss, slightly reduce calories or add extra activity.",
    "fit.analysisGainingMuscle": "You are gaining weight, consistent with your muscle-building goal.",
    "fit.analysisLosingMuscleGoal": "You are losing weight while trying to gain muscle — increase calories and protein.",
    "fit.analysisMaintaining": "Weight is well maintained — keep your current macros and training volume.",
    "fit.analysisHighFreq": "Excellent training frequency (4+ sessions/week). Watch recovery and sleep.",
    "fit.analysisMidFreq": "Good consistency (2–3 sessions/week). Aim for one extra session for faster results.",
    "fit.analysisLowFreq": "Low training frequency. Try to log at least 2 sessions per week.",
    "fit.analysisWaistDown": "Waist down by {n} cm — clear sign of fat loss even if the scale moves slowly.",
    "fit.analysisBmiHigh": "BMI above the healthy range — prioritize sustained caloric deficit and strength training.",
    "fit.analysisNoData": "Log a few sessions and progress photos to unlock a personalized analysis.",
    "type.walking": "Walking",
    "type.running": "Running",
    "type.cycling": "Cycling",
    "type.swimming": "Swimming",
    "type.weights": "Weight training",
    "type.calisthenics": "Calisthenics",
    "type.hiit": "HIIT",
    "type.crossfit": "CrossFit",
    "type.yoga": "Yoga",
    "type.pilates": "Pilates",
    "type.stretching": "Stretching / mobility",
    "type.dance": "Dance",
    "type.boxing": "Boxing / martial arts",
    "type.rowing": "Rowing",
    "type.elliptical": "Elliptical",
    "type.stairs": "Stair climber",
    "type.hike": "Hiking",
    "type.football": "Football / soccer",
    "type.basketball": "Basketball",
    "type.tennis": "Tennis / padel",
    "type.functional": "Functional training",
    "type.other": "Other",
  },
};

// Spanish strings for Fit
Object.assign(translations.es, {
  "fit.title": "Paraguachi Fit",
  "fit.subtitle": "Planificador mensual de entrenamientos alineado con tus comidas",
  "fit.monthlyOnly": "Disponible solo para suscriptores mensuales",
  "fit.upgrade": "Mejora a mensual para desbloquearlo",
  "fit.activate": "Generar mi rutina",
  "fit.regenerate": "Generar nueva rutina",
  "fit.generating": "Diseñando tu rutina...",
  "fit.generated": "¡Rutina lista!",
  "fit.error": "No se pudo generar la rutina",
  "fit.tabRoutine": "Rutina",
  "fit.tabSessions": "Sesiones",
  "fit.tabProgress": "Progreso",
  "fit.daysWeek": "días/semana",
  "fit.duration": "Duración",
  "fit.weeks": "semanas",
  "fit.week": "Semana",
  "fit.session": "Sesión",
  "fit.warmup": "Calentamiento",
  "fit.cooldown": "Enfriamiento",
  "fit.exercises": "Ejercicios",
  "fit.macroAdjust": "Tip de macros",
  "fit.tips": "Consejos generales",
  "fit.logSession": "Registrar entrenamiento",
  "fit.uploadScreenshot": "Sube screenshot (Apple Watch, Fitbit, Garmin...)",
  "fit.analyzing": "Analizando con IA...",
  "fit.aiDetected": "IA detectó",
  "fit.kcal": "kcal",
  "fit.km": "km",
  "fit.minutes": "min",
  "fit.type": "Tipo",
  "fit.device": "Dispositivo",
  "fit.notes": "Notas",
  "fit.save": "Guardar sesión",
  "fit.savedSession": "Sesión registrada",
  "fit.noSessions": "Aún no has registrado sesiones",
  "fit.uploadProgress": "Subir foto de progreso",
  "fit.photoType": "Tipo de foto",
  "fit.photoStart": "Inicial",
  "fit.photoProgress": "Progreso",
  "fit.photoCurrent": "Actual",
  "fit.weight": "Peso (kg)",
  "fit.waist": "Cintura (cm)",
  "fit.savedPhoto": "Foto subida",
  "fit.noPhotos": "Aún no hay fotos de progreso",
  "fit.deviceConnect": "Integraciones con dispositivos",
  "fit.deviceComingSoon": "Sincronización directa próximamente",
  "fit.uploadAnyDevice": "Sube screenshots de cualquier dispositivo — la IA extrae tus datos automáticamente",
  "fit.dashboardCard": "Paraguachi Fit",
  "fit.dashboardOpen": "Abrir entrenamiento",
  "fit.dashboardDesc": "Tu rutina mensual alineada con tus comidas",
  "fit.location": "Ubicación",
  "fit.intensity": "Intensidad",
  "fit.indoor": "Indoor",
  "fit.outdoor": "Outdoor",
  "fit.intLow": "Baja",
  "fit.intModerate": "Moderada",
  "fit.intHigh": "Alta",
  "fit.selectType": "Selecciona la actividad",
  "fit.manualHint": "Elige el tipo de actividad para que ajustemos tus macros con precisión",
  "fit.sessionDuration": "Duración por sesión",
  "fit.express": "Express",
  "fit.days": "días",
  "fit.watchDemo": "Ver demo",
  "fit.rest": "Descanso",
  "fit.howTo": "Cómo realizarlo",
  "fit.demoHint": "Abre en YouTube o MuscleWiki para ver demostraciones reales",
  "fit.openYouTube": "Ver en YouTube",
  "fit.openMuscleWiki": "Ver en MuscleWiki",
  "fit.gender": "Género",
  "fit.male": "Masculino",
  "fit.female": "Femenino",
  "fit.level": "Nivel",
  "fit.levelBeginner": "Principiante",
  "fit.levelIntermediate": "Intermedio",
  "fit.levelAdvanced": "Avanzado",
  "fit.yourGoal": "Tu objetivo actual",
  "fit.setDesiredWeight": "Definir peso deseado",
  "fit.editGoal": "Editar",
  "fit.desiredWeight": "Peso deseado",
  "fit.startWeight": "Inicial",
  "fit.currentWeight": "Actual",
  "fit.goalProgress": "Progreso hacia la meta",
  "fit.weightChange": "{kg} kg vs inicial",
  "fit.noWeightChange": "Aún sin cambios de peso",
  "fit.remaining": "Faltan {kg} kg",
  "fit.height": "Altura",
  "fit.dailyKcal": "Meta diaria",
  "fit.bmiUnder": "bajo peso",
  "fit.bmiNormal": "saludable",
  "fit.bmiOver": "sobrepeso",
  "fit.bmiObese": "obesidad",
  "fit.last30Days": "Últimos 30 días",
  "fit.statSessions": "Sesiones",
  "fit.statTime": "Tiempo total",
  "fit.statKcal": "Quemadas",
  "fit.statKm": "Distancia",
  "fit.perWeek": "/semana",
  "fit.technicalAnalysis": "Análisis técnico",
  "fit.analysisLosing": "Estás bajando de peso de forma constante — tu déficit calórico está funcionando.",
  "fit.analysisGainingFatGoal": "El peso sube a pesar de tu objetivo de pérdida de grasa. Revisa porciones y adherencia al plan.",
  "fit.analysisStable": "El peso está estable. Si quieres acelerar la pérdida, reduce ligeramente calorías o suma actividad extra.",
  "fit.analysisGainingMuscle": "Estás ganando peso, alineado con tu objetivo de ganar músculo.",
  "fit.analysisLosingMuscleGoal": "Estás perdiendo peso mientras buscas ganar músculo — sube calorías y proteína.",
  "fit.analysisMaintaining": "Peso bien mantenido — conserva tus macros y volumen de entrenamiento actuales.",
  "fit.analysisHighFreq": "Excelente frecuencia de entrenamiento (4+ sesiones/semana). Cuida la recuperación y el sueño.",
  "fit.analysisMidFreq": "Buena constancia (2–3 sesiones/semana). Sumar una sesión más acelera resultados.",
  "fit.analysisLowFreq": "Baja frecuencia de entrenamiento. Intenta registrar al menos 2 sesiones por semana.",
  "fit.analysisWaistDown": "Cintura −{n} cm — señal clara de pérdida de grasa aunque la báscula se mueva lento.",
  "fit.analysisBmiHigh": "IMC por encima del rango saludable — prioriza déficit calórico sostenido y entrenamiento de fuerza.",
  "fit.analysisNoData": "Registra algunas sesiones y fotos de progreso para desbloquear un análisis personalizado.",
  "type.walking": "Caminata",
  "type.running": "Correr",
  "type.cycling": "Ciclismo",
  "type.swimming": "Natación",
  "type.weights": "Pesas / fuerza",
  "type.calisthenics": "Calistenia",
  "type.hiit": "HIIT",
  "type.crossfit": "CrossFit",
  "type.yoga": "Yoga",
  "type.pilates": "Pilates",
  "type.stretching": "Estiramiento / movilidad",
  "type.dance": "Baile",
  "type.boxing": "Boxeo / artes marciales",
  "type.rowing": "Remo",
  "type.elliptical": "Elíptica",
  "type.stairs": "Escaladora",
  "type.hike": "Senderismo",
  "type.football": "Fútbol",
  "type.basketball": "Baloncesto",
  "type.tennis": "Tenis / pádel",
  "type.functional": "Entrenamiento funcional",
  "type.other": "Otro",
});

export function I18nProvider({ children }: { children: ReactNode }) {
  const [lang, setLangState] = useState<Lang>(detectLanguage);

  const setLang = (l: Lang) => {
    setLangState(l);
    localStorage.setItem("lang", l);
    document.documentElement.lang = l;
  };

  useEffect(() => {
    document.documentElement.lang = lang;
  }, []);

  const t = (key: string): string => translations[lang][key] || translations.es[key] || key;

  return (
    <I18nContext.Provider value={{ lang, setLang, t }}>
      {children}
    </I18nContext.Provider>
  );
}

const fallback: I18nContextType = {
  lang: detectLanguage(),
  setLang: () => {},
  t: (key: string) => translations[detectLanguage()]?.[key] || translations.es[key] || key,
};

export function useI18n() {
  const ctx = useContext(I18nContext);
  return ctx || fallback;
}
